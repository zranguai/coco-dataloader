...
class SequentialSampler(Sampler):
    ...
    def __iter__(self):
        # 返回迭代器，不然无法 for .. in ..
        return iter(range(len(self.data_source)))  # 所以说__getitem__必须实现__len__方法
...
class RandomSampler(Sampler):
    ...
    def __iter__(self):
        ...
        if self.replacement:
            ...
            yield from torch.randint(high=n, size=(self.num_samples % 32,), dtype=torch.int64).tolist()  # yield剩下的
        else:
            # 无放回抽样
